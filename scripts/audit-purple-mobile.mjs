...
page.on("requestfailed", request => addFinding("high", "network", "Resource failed to load", `${request.method()} ${request.url()} — ${request.failure()?.errorText || "unknown"}`));
...
try {
  ...
  await shot("startup-cleared", "checked", "Dismissed any splash, tutorial, or entry overlays that exposed a usable control.");
  ...
  if (layout.contentHeight <= layout.viewportHeight + 2) addFinding("medium", "scrolling", "The page has no vertical scroll range", `${layout.contentHeight}px content / ${layout.viewportHeight}px viewport`);
  ...
  if (layout.contentHeight > layout.viewportHeight + 20 && scrollEnd === scrollStart) addFinding("critical", "scrolling", "Vertical scrolling is blocked on phone", `scrollY remained ${scrollEnd}`);
  await page.evaluate(() => window.scrollTo(0, 0));
  await shot("scroll-test", scrollEnd > scrollStart || layout.contentHeight <= layout.viewportHeight + 20 ? "working" : "broken", "Checked that the page can move vertically without widening beyond the screen.");

  let openedParts = await clickFirst([/^\+\s*part$/i, /parts library/i, /show parts library/i, /^parts$/i]);
  ...
  await page.waitForTimeout(500);
  await shot("parts-library", openedParts ? "opened" : "broken", "Attempted to open the main library used to build a scene.");
  ...
  if (benchVisible && rowCount) {
    ...
    if (rb && bb) {
      ...
      await page.mouse.move(bb.x + bb.width * 0.62, bb.y + bb.height * 0.48, { steps: 12 });
      await page.mouse.up();
  ...
  if (await vessel.isVisible().catch(() => false) && await chemicalRows.count()) {
    const vb = await vessel.boundingBox();
    const cb = await chemicalRows.first().boundingBox();
    if (vb && cb) {
      const beforeText = await vessel.innerText().catch(() => "");
      await page.mouse.move(cb.x + cb.width / 2, cb.y + cb.height / 2);
      await page.mouse.down();
      await page.mouse.move(vb.x + vb.width / 2, vb.y + vb.height / 2, { steps: 12 });
      await page.mouse.up();
      await page.waitForTimeout(700);
  ...
  await shot("add-chemical", addedChemical ? "working" : "not working", "Attempted the core action: drop a chemical into a vessel and look for a visible result.");
  ...
  const report = {
    ...
    verdict: critical.length ? "DO NOT DEPLOY" : findings.length ? "DEPLOY ONLY AFTER FIXES" : "CORE FLOW PASSED",
    steps,
    findings
  };
